...
async function loadPosts(page = 1) {
  ...
  if (!data || data.length === 0) {
    tbody.innerHTML = `<tr><td colspan="4" class="empty-msg">${searchKeyword ? '검색 결과가 없습니다.' : '아직 게시글이 없습니다. 첫 글을 작성해보세요!'}</td></tr>`;
    renderPagination(0);
    return;
  }
...
async function openPost(postId) {
  const { data, error } = await sb.from('free_posts').select('*').eq('id', postId).single();
...
document.getElementById('btn-write').addEventListener('click', () => {
  ...
  document.getElementById('input-category').value       = '일반';
  showView('view-write');
});
...
document.getElementById('btn-delete').addEventListener('click', async () => {
  ...
  const { error } = await sb.from('free_posts').delete().eq('id', currentPost.id);
